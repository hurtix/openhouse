...
<div class="wrapper relative pt-10" id="archive-wrapper" style="background: linear-gradient(-180deg, rgb(33, 37, 41) 25vh, var(--color-gray-100) 25vh);">
    <div class="container mx-auto px-4 md:px-0" id="content" tabindex="-1">
        <div class="flex flex-wrap">
            <div class="flex justify-between items-center mb-4 w-full">
                <button id="btn-filtro" type="button" class="btn text-sm md:text-base inline-flex cursor-pointer text-white py-3 px-4 leading-none rounded-full border border-gray-200 hover:bg-white hover:text-[var(--dark)]" data-bs-toggle="button" data-filter-value="<?php echo esc_attr($term->slug); ?>"></button>
                <span id="resultados" class="text-white"></span>
            </div>
            <div class="flex items-start gap-8">
                <div id="wrapper-filtro" class="hidden mb-20 shadow-lg bg-white border border-gray-200 rounded-none md:rounded p-4">
                ...
                <main id="main" class="site-main grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-4 w-full pb-20">
                    <?php
                    if (have_posts()) {
                        while (have_posts()) {
                            the_post();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateButtonContent() {
            ...
            btnFiltro.innerHTML = isOpen ? 
                `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-lg me-1" viewBox="0 0 16 16"><path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8z"/></svg> Cerrar filtro` : 
...
<?php } else { ?>
    btnFiltro.addEventListener('click', function() {
            ...
            main.classList.toggle('md:grid-cols-4');
            main.classList.toggle('md:grid-cols-3');
        });
<?php } ?>
